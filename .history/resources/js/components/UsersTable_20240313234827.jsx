                    ...
                    <tbody>
                        {users.map((user, index) => (
                            <tr key={index} className="border-b transition duration-300 ease-in-out hover:bg-neutral-100 dark:border-neutral-500 dark:hover:bg-neutral-600">
                                ...
                                <td className="whitespace-nowrap px-10 py-4">
                                <Link to={`/details/${user.id}`} className="flex flex-row-reverse">
                                    <SecondaryButton onClick={() => onUpdateUser(user.id)}>Update</SecondaryButton>
                                </Link>
                                    <DangerButton onClick={() => onDeleteUser(user.id)}>Delete</DangerButton>
                                </td>
                            </tr>
                        ))}
                    </tbody>
