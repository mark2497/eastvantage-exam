...
export default function RegistrationPage() {
        ...
        axios.get(`/details/${id}`)
            .then(({data} = response) => {
                setFormData({
                    ...
                    checkedRoles: data.user.roles.map(role => role.role.role),
                    mode: 'update'
                })
            })
            .catch((error) => {
                if (error.response.status == 422) {
                    return distributeErrors(error.response.data.errors)
                }
                return Alerts.dangerMessageWithRedirect(error.response.data.message, "/")
            })
    }
        ...
        if (validate()) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.post(formData.mode == 'update' ? '/update' : '/new', formData)
                    .then(({data} = response) => {
    ...
    const distributeErrors =  (errors) => {
        forEach(Object.keys(errors), function(error){
            if (error == "email") {
                setFormData({
                    ...formData,
                    errors: {
                        ...formData.errors,
                        email: errors[error][0],
                    }
                });
            }
            if (error == 'fullName') {
                setFormData({
                    ...
                    errors: {
                        ...formData.errors,
                        fullName: errors[error][0],
                    }
                });
            }
            if (error == 'checkedRoles') {
                setFormData({
                    ...formData,
                    errors: {
                        ...formData.errors,
    ...
    return (
        <Layout>
            <form className="mx-auto">
                ...
                <SecondaryButton onClick={()=>history.back()}>Cancel</SecondaryButton>
            </form>
        </Layout>
    )
}
