    ...
    return (
        <Layout>
            <form className="mx-auto">
                ...
                {roles.map((item) => (
                    <label className="flex items-center p-4 border border-gray-300 rounded-md shadow-sm hover:shadow-md transition duration-300 ease-in-out cursor-pointer">
                    <input
                        key={`role_${item}`}
                        type="checkbox"
                        className="form-checkbox mr-3 h-5 w-5 text-blue-500"
                        isChecked={checkedRoles.includes(item)}
