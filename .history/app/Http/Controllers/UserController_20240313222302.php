...
class UserController extends Controller
{
    public function index(UserListRequest $request) : LengthAwarePaginator {
        $role_id = Role::where('name', $request->get('role'))->pluck( 'id' )->first();
        dd($role_id);
        $usersByRole = User::whereHas('roles', function($role) use ($request) {
                $role->when($request->get('role'), fn($q) => $q->where('role_id', $request->get('role')), fn($q) => $q);
            })
            ->with('roles')
            ->paginate(
                $request->get('size'),
